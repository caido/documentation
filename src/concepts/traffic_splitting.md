...
- [GraphQL](/concepts/graphql.md) API operations resulting from interactions with the Caido GUI (_client component_).
...
**Yes**: The request is generated by a [proxy-aware](/concepts/web_traffic.md#proxy-aware-clients) client. Caido establishes TCP/TLS connections with both the client and the destination server.
...
| GET / HTTP/1.1 <br> Host: 127.0.0.1 | `http://127.0.0.1:8080` | API | 502 Bad Gateway | Failed to connect: 127.0.0.1:80 |
| GET / HTTP/1.1 <br> Host: www.google.com | `http://127.0.0.1:8080` | Proxy | 200 OK | |
| GET / HTTP/1.1 <br> Host: www.google.com | `http://www.google.com` | Proxy | 200 OK | |
| GET / HTTP/1.1 | `http://www.google.com` | Proxy | 200 OK | |

If [invisible proxying](/tutorials/invisible_proxy.md) is enabled and configured to proxy traffic generated by proxy-unaware [thick clients](/concepts/web_traffic.md#thick-clients) the behavior will be the same.

However, without a [DNS rewrite](/guides/dns_rewrites.md), Caido will not forward the request to the IP address of the destination server. The destination will resolve to the API, resulting in 400 Bad Request responses due to malformed requests.
