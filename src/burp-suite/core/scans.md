...
Use the community **Scanner** plugin for automated vulnerability scanning in Caido. Caido does not ship a built-in scanner like Burp Pro; active and passive scanning is provided by this plugin and can be extended with custom checks.
...
Configure scan behavior through the **Scanner** plugin's custom check definitions and native **Workflows**. Insertion points and check selection are plugin- and workflow-specific rather than a unified scan configuration UI.
...
Store credentials in native **Environment Variables** and record login flows manually in **Replay**. Caido does not support recorded login sequences or a login configuration library like Burp; authenticated scanning requires manual session setup.
...
Caido has no native resource pool for throttling scan concurrency. Throttle **Automate** campaigns manually or add delays in workflows to control request rate.
...
Run active scans with the **Scanner** plugin and targeted fuzzing with native **Automate**. Caido does not offer a single full crawl-and-audit pipeline like Burp's integrated scanner; combine plugins and native tools for similar coverage.
