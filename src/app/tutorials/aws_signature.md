...
Then, by using [workflows in Replay](/app/guides/replay_environment_variables.md), you can achieve continuous, uninterrupted testing without manually updating expired sessions.
...
With these values, [create environment variables](https://docs.caido.io/app/tutorials/aws_signature.html) in the `Global` environment with the following names:
...
1. **Click** on the `Javascript` node to access its editor and ensure the `$convert_start.data` is [referenced as input data](/app/guides/workflows_references.md).
